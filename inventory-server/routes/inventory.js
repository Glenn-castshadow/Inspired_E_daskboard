...
router.post('/', (req, res) => {
  const { item_type, material, width, height, thickness, quantity, sku, notes, unit_cost } = req.body;
  ...
  const info = db.prepare(
    `INSERT INTO inventory
       (item_type, material, width, height, thickness, quantity, sku, notes, unit_cost, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(
...
router.put('/:id', (req, res) => {
  ...
  const info = db.prepare(
    `UPDATE inventory
     SET item_type=?, material=?, width=?, height=?, thickness=?, quantity=?, sku=?, notes=?, unit_cost=?, updated_at=?
     WHERE id=?`
  ).run(item_type, material, width, height, thickness, quantity, sku ?? '', notes ?? '', unit_cost ?? 0, now(), req.params.id);
